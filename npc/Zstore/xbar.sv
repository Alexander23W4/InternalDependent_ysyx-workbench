...
module ysyx_26040135_AXI_XBAR (
        ...
        if (reset) begin
            ...
            if (r_state == R_IDLE) begin
                if (m0.arvalid) begin
                    ...
                    r_latched_arid <= m0.arid;
                    r_latched_arlen <= m0.arlen;
                    r_latched_arsize <= m0.arsize;
                    r_latched_arburst <= m0.arburst;
                    ...
                    r_latched_arid <= m1.arid;
                    r_latched_arlen <= m1.arlen;
                    r_latched_arsize <= m1.arsize;
                    r_latched_arburst <= m1.arburst;
    ...
    assign s0.arid     = (r_latched_slave_sel == 2'b01) ? r_latched_arid : '0;
    assign s0.arlen    = (r_latched_slave_sel == 2'b01) ? r_latched_arlen : '0;
    assign s0.arsize   = (r_latched_slave_sel == 2'b01) ? r_latched_arsize : '0;
    assign s0.arburst  = (r_latched_slave_sel == 2'b01) ? r_latched_arburst : '0;
    assign s0.arvalid  = (r_latched_slave_sel == 2'b01) && (r_state == R_BUSY);

    assign s1.araddr   = (r_latched_slave_sel == 2'b10) ? r_latched_addr : '0;
    assign s1.arid     = (r_latched_slave_sel == 2'b10) ? r_latched_arid : '0;
    assign s1.arlen    = (r_latched_slave_sel == 2'b10) ? r_latched_arlen : '0;
    assign s1.arsize   = (r_latched_slave_sel == 2'b10) ? r_latched_arsize : '0;
    assign s1.arburst  = (r_latched_slave_sel == 2'b10) ? r_latched_arburst : '0;
    ...
    assign m0.rid     = (r_latched_is_m0) ? ((r_latched_slave_sel == 2'b01) ? s0.rid : (r_latched_slave_sel == 2'b10) ? s1.rid : '0) : '0;
    assign m0.rlast   = (r_latched_is_m0) ? ((r_latched_slave_sel == 2'b01) ? s0.rlast : (r_latched_slave_sel == 2'b10) ? s1.rlast : 1'b1) : 1'b1;
    ...
    assign m1.rid     = (!r_latched_is_m0) ? ((r_latched_slave_sel == 2'b01) ? s0.rid : (r_latched_slave_sel == 2'b10) ? s1.rid : '0) : '0;
    assign m1.rlast   = (!r_latched_is_m0) ? ((r_latched_slave_sel == 2'b01) ? s0.rlast : (r_latched_slave_sel == 2'b10) ? s1.rlast : 1'b1) : 1'b1;
...
-------------------------------------------------------------------------------------------------------------*/
    ...
    logic [3:0]  w_latched_awid;
    logic [7:0]  w_latched_awlen;
    logic [2:0]  w_latched_awsize;
    logic [1:0]  w_latched_awburst;
        ...
        if (reset) begin
            ...
            if (w_state == W_IDLE) begin
                if (m0.awvalid) begin
                    ...
                    w_latched_awid <= m0.awid;
                    w_latched_awlen <= m0.awlen;
                    w_latched_awsize <= m0.awsize;
                    w_latched_awburst <= m0.awburst;
                    ...
                    w_latched_awid <= m1.awid;
                    w_latched_awlen <= m1.awlen;
                    w_latched_awsize <= m1.awsize;
                    w_latched_awburst <= m1.awburst;
    ...
    assign s0.awid     = (w_latched_slave_sel == 2'b01) ? w_latched_awid : '0;
    assign s0.awlen    = (w_latched_slave_sel == 2'b01) ? w_latched_awlen : '0;
    assign s0.awsize   = (w_latched_slave_sel == 2'b01) ? w_latched_awsize : '0;
    assign s0.awburst  = (w_latched_slave_sel == 2'b01) ? w_latched_awburst : '0;
    assign s0.awvalid  = (w_latched_slave_sel == 2'b01) && (w_state == W_BUSY);

    assign s1.awaddr   = (w_latched_slave_sel == 2'b10) ? w_latched_addr : '0;
    assign s1.awid     = (w_latched_slave_sel == 2'b10) ? w_latched_awid : '0;
    assign s1.awlen    = (w_latched_slave_sel == 2'b10) ? w_latched_awlen : '0;
    assign s1.awsize   = (w_latched_slave_sel == 2'b10) ? w_latched_awsize : '0;
    assign s1.awburst  = (w_latched_slave_sel == 2'b10) ? w_latched_awburst : '0;
    ...
    assign s0.wlast  = (w_latched_slave_sel == 2'b01) ? (w_latched_is_m0 ? m0.wlast : m1.wlast) : 1'b1;
    ...
    assign s1.wlast  = (w_latched_slave_sel == 2'b10) ? (w_latched_is_m0 ? m0.wlast : m1.wlast) : 1'b1;
    ...
    assign m0.bid     = (w_latched_is_m0) ? ((w_latched_slave_sel == 2'b01) ? s0.bid : (w_latched_slave_sel == 2'b10) ? s1.bid : '0) : '0;
    ...
    assign m1.bid     = (!w_latched_is_m0) ? ((w_latched_slave_sel == 2'b01) ? s0.bid : (w_latched_slave_sel == 2'b10) ? s1.bid : '0) : '0;
