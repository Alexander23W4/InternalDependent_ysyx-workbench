...
// 0000000 01011 01111 001 01111 0110011 
  ...
  INSTPAT("0000000 ????? ????? 010 ????? 01100 11", sll    , R, R(rd) = src1 << (src2 & 0x1F));
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->snpc, s->dnpc = (src1 + imm) & ~1); // guest addr
