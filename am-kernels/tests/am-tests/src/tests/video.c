...
static uint32_t canvas[N][N];  // N*N canvas
static int used[N][N];

static uint32_t color_buf[32 * 32];  // ??

void redraw() {
  int w = io_read(AM_GPU_CONFIG).width / N;   // one piece of width
  int h = io_read(AM_GPU_CONFIG).height / N;
  int block_size = w * h;                     // total buffer
  assert((uint32_t)block_size <= LENGTH(color_buf));   // buffer not large than 32*32
